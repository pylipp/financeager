...
def migrate_pocket(pocket_name, data_dir):
    ...
    for entry in standard_entries:
        ...
        sqlite_pocket.db_interface._conn.execute(
            f"INSERT INTO {DEFAULT_TABLE} (eid, name, date, category, value) VALUES (?, ?, ?, ?, ?)",
            (doc_id, entry_data['name'], entry_data['date'], entry_data.get('category'), entry_data['value'])
        )
        standard_count += 1
    ...
    for entry in recurrent_entries:
        ...
        # Insert with specific eid
        sqlite_pocket.db_interface._conn.execute(
            f"INSERT INTO {RECURRENT_TABLE} (eid, name, start, end, frequency, category, value) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (doc_id, entry_data['name'], entry_data['start'], entry_data.get('end'), 
             entry_data['frequency'], entry_data.get('category'), entry_data['value'])
        )
        recurrent_count += 1
