...
def suite():
    ...
    suite.addTest(
        unittest.TestSuite(map(TinyDbPeriodStandardEntryTestCase, tests)))
    tests = [
    ...
    suite.addTest(
        unittest.TestSuite(map(TinyDbPeriodRecurrentEntryTestCase, tests)))
    tests = [
        'test_no_future_elements_created',
    ]
    suite.addTest(
        unittest.TestSuite(map(TinyDbPeriodRecurrentEntryNowTestCase, tests)))
    tests = [
...
class TinyDbPeriodStandardEntryTestCase(unittest.TestCase):
    ...
    def test_update_standard_entry(self):
        ...
        self.period.update_entry(
            eid=self.eid, name="Trekking Bicycle", value=None)
        element = self.period.get_entry(eid=self.eid)
...
class ValidationModelTestCase(unittest.TestCase):
    ...
    def test_valid_standard_entry(self):
        entry = StandardEntryValidationModel({
            "name": "entry",
            "value": 5,
            "date": "05-01"
        })
    ...
    def test_valid_recurrent_entry(self):
        entry = RecurrentEntryValidationModel({
            "name": "rent",
            "value": -400,
            "frequency": "monthly",
            "start": "01-02"
    ...
    def test_invalid_recurrent_entry(self):
        with self.assertRaises(DataError) as context:
            model = RecurrentEntryValidationModel({
                "name": "rent",
                "value": -400,
                "frequency": "yaerly",
                "start": "01-02"
